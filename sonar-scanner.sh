...
if [ $1 == 'run' ]; then
	if [ -x "$(sonar-scanner -v)" ]; then
		...
		sonar-scanner -Dsonar.projectKey=twitter-tracker -Dsonar.sources=. -Dsonar.host.url=http://aminsep.disi.unibo.it:9000 -Dsonar.login=88d33c465a8db4b6c686f9987d5e29a89fc25798
	fi
elif [ $1 == 'install' ]; then
	if [ -z $2 ]; then
